...
inputLines = fmap lines $ readFile "inputs/input7.txt"

-- Star 1
data Directory = Dir String (Map String Directory) (Maybe Directory) |
                 File Integer
                deriving Show

data Command = Ls [String] | Cd String 
               deriving Show

updateSubtree (Dir name files maybeParent) newFiles = case maybeParent of
    Nothing -> (Dir name newFiles maybeParent)
    Just parent@(Dir _ parentFiles parentMaybeParent) ->
        let newDirectory = updateChildren newParent (Dir name newFiles newParent)
            newParentFiles = insert name newDirectory parentFiles
            newParent = Just (updateSubtree parent newParentFiles)
        in newDirectory

updateChildren newParent directory@(Dir name files _) = newDirectory
    where newDirectory = (Dir name newFiles newParent)
          newFiles = fmap (updateChildren (Just newDirectory)) files
updateChildren _ file = file

parseCommands (l:ls) (unfinishedLsLines)
    | length tokens == 0 = parseCommands ls (unfinishedLsLines ++ [l])
    | (tokens !! 0) == "cd" = finishedLs ++ ((Cd $ tokens !! 2):parseCommands ls [])
    | (tokens !! 0) == "ls" = finishedLs ++ (parseCommands ls [])
    where (_, _, _, tokens) = (l =~ "\\$ (cd|ls)( (.*))?") :: (String, String, String, [String])
          finishedLs
            | length unfinishedLsLines > 0 = [Ls unfinishedLsLines]
            | otherwise = []
parseCommands [] unfinishedLsLines = if length unfinishedLsLines > 0 then [Ls unfinishedLsLines] else []

executeLsLine ('d':'i':'r':' ':name) directory@(Dir _ files _) = updateSubtree directory (insert name (Dir name Data.Map.empty (Just directory)) files)
executeLsLine fileLine directory@(Dir _ files _) = updateSubtree directory (insert name (File (read size)) files)
    where (_, _, _, [size, name]) = (fileLine =~ "([0-9]*) (.+)") :: (String, String, String, [String])

executeCommand (Cd "..") (Dir _ _ (Just parent)) = parent
-- This shouldn't happen!
-- executeCommand (Cd "..") (Dir name files Nothing) = newParent
--     where newParent = Dir "" (fromList [(name, newDirectory)]) Nothing
--           newDirectory = (Dir name files (Just newParent))
executeCommand (Cd "/") _ = (Dir "/" Data.Map.empty Nothing)
executeCommand (Cd name) directory@(Dir currentName files currentParent) = case Data.Map.lookup name files of
    Just child -> (updateChildren (Just directory) child)
    Nothing -> child
        where child = (Dir name Data.Map.empty (Just (Dir currentName (insert name child files) currentParent)))
executeCommand (Ls (l:ls)) directory = executeCommand (Ls ls) (executeLsLine l directory)
executeCommand (Ls []) directory = directory

buildFilesystem (c:cs) currentTree = buildFilesystem cs (executeCommand c currentTree)
buildFilesystem [] currentTree = currentTree

directoriesTotalSizes (Dir _ files _) = [sum (fmap fileSize files) + sum [sizes !! 0 | sizes <- subdirectoriesTotalSizes]] ++ (concat subdirectoriesTotalSizes)
    where fileSize (File size) = size
          fileSize _ = 0
          subdirectoriesTotalSizes = [directoriesTotalSizes subDirectory | (_, subDirectory@(Dir _ _ _)) <- toList files]

root (Dir _ _ (Just parent)) = root parent
root rootDirectory = rootDirectory

tree x = _tree "" x where
    _tree prefix (Dir name files _) = do
        putStrLn $ prefix ++ name ++ "/"
        mapM_ (_tree (prefix ++ "| ")) [subdir | (_, subdir) <- (Data.Map.toList files)]
    _tree prefix (File size) = do
        putStrLn $ prefix ++ "File (" ++ (show size) ++ ")"

computeSolution1 inputLines = sum [size | size <- Prelude.drop 1 (directoriesTotalSizes rootDir), size <= 100000]
    where commands = parseCommands inputLines []
          filesystem = buildFilesystem commands (Dir "" Data.Map.empty Nothing)
          rootDir = root filesystem

numFiles (Dir _ files _) = 1 + sum [numFiles f | (_, f) <- toList files]
numFiles (File _) = 1

nthChild (Dir _ files _) i = file 
    where (_, file) = (toList files) !! i

name (Dir name _ _) = name
name (File size) = "File of size " ++ (show size)

fullPaths x = _fullPaths "" x
    where _fullPaths prefix (Dir name files _) = (fmap ((prefix ++ name ++ "/")++) (keys files)) ++ (concat (fmap (_fullPaths (prefix ++ "/" ++ name)) [file | (_, file) <- toList files]))
          _fullPaths _ _ = []

fs = root $ buildFilesystem (parseCommands l []) (Dir "" Data.Map.empty Nothing)

firstNTree i = tree $ root $ buildFilesystem (parseCommands (Prelude.take i l) []) (Dir "" Data.Map.empty Nothing)

d = d1 where d1 = (Dir "" (fromList [("x",d2)]) Nothing); d2 = (Dir "x" (fromList []) (Just d1))

-- l = ["$ cd /", "$ ls", "187585 dgflmqwt.srm", "dir abc", "$ cd abc", "$ ls", "129 a.b", "$ cd ..", "$ cd def", "$ ls", "11 b.c", "$ cd ..", "$ cd abc", "$ ls", "12 b.c", "13 b.c", "dir xy", "$ cd xz", "$ ls", "31 yx", "$ cd ..", "$ cd .."]


-- Star 2

computeSolution2 inputLines =  minimum [directorySize | directorySize <- subdirectorySizes, directorySize >= requiredSpace]
    where (totalSize:subdirectorySizes) = directoriesTotalSizes rootDir
